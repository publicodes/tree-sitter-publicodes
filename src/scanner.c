...
// CAREFUL : This enum should be in the same order as the `externals` array in
// the grammar.js file
enum TokenType {
  INDENT,
  DEDENT,
...
static bool parse_paragraph(void *payload, TSLexer *lexer,
  ...
  while (lexer->lookahead) {
    ...
    // Check if the indent length of the following line less than the paragraph
    // indent length
...
bool tree_sitter_publicodes_external_scanner_scan(void *payload, TSLexer *lexer,
                                                  const bool *valid_symbols) {
...
void tree_sitter_publicodes_external_scanner_deserialize(void *payload,
                                                         const char *buffer,
                                                         unsigned length) {
...
unsigned tree_sitter_publicodes_external_scanner_serialize(void *payload,
                                                           char *buffer) {
  Array(int) *indents = payload;
  uint32_t iter = 1;
  size_t size = 0;
  for (; iter < indents->size && size < TREE_SITTER_SERIALIZATION_BUFFER_SIZE;
       ++iter) {
